...
price); 
void insertmid(int pos, int data, char foodname[25], int quantity,
float price);
void insertend(int data, char foodname[25], int quantity, float
price);
...
printf(" 7. BackupSystem\n\n");
Sleep(250);
pre(4);

printf(" 8. Instant OrderPreview\n\n");
Sleep(250);pre(4);
...
cls(); br(5);pre(3); printf(" Food Serial Already Exist, Please Re-Enter "); 
Sleep(2000);
goto foodserial;
}
fprice:
...
br(2);pre(4); printf("Backup Successfull..."); 
Sleep(1500);
...
temp = temp->next;
if(temp==NULL){
br(2); pre(4); echo("Please Choice From List ");
...
cls(); middle1();pre(3); printf("Quantity Can not be Zero ");
 Sleep(2000);
cls();
goto foodlist;
}
